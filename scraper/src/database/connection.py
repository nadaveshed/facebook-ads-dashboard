...
class AdRepository:
    ...
    def insert_ad(self, ad_data: Dict[str, Any]) -> int:
        with self.db.get_cursor() as cursor:
            cursor.execute("""
                INSERT INTO ads (ad_id, status, platforms, start_date, end_date, 
                                asset_type, asset_path, ad_content, advertiser_name)
                VALUES (%(ad_id)s, %(status)s, %(platforms)s, %(start_date)s, %(end_date)s,
                        %(asset_type)s, %(asset_path)s, %(ad_content)s, %(advertiser_name)s)
    ...
    def insert_ads_batch(self, ads: List[Dict[str, Any]]) -> int:
        ...
        with self.db.get_cursor() as cursor:
            values = [
                (
                    ad['ad_id'], ad['status'], ad['platforms'], 
                    ad.get('start_date'), ad.get('end_date'),
                    ad.get('asset_type', 'none'), ad.get('asset_path'),
                    ad.get('ad_content'), ad.get('advertiser_name', 'Nike')
                )
                for ad in ads
            ]
